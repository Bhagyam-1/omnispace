    ...
    return (
        <article className='flex justify-center w-full xs:px-4'>
            <Card className='h-fit flex flex-col gap-3 bg-transparent border-0 w-full xs:w-fit'>
                <CardHeader className='flex items-center gap-3 px-2 xs:px-6 w-full'>
                    <Link href={`/connect/profile/${post?.user?.userName}`} className='flex items-center gap-2'>
                        <img src={post?.user?.image} alt={`Profile icon of ${post?.user?.userName}`} className='h-8 w-8 rounded-full object-cover' />
                ...
                <CardContent className='p-0 xs:px-6'>
                    {
                        post?.content?.type === PostType.IMAGE && (
                            <img src={post?.content?.image?.url} alt={`A post by ${post?.user?.userName} with caption ${post?.content?.image?.caption}`}
                            className='h-[400px] md:h-[500px] w-full xs:w-[475px] object-cover xs:rounded' />
                        )
                    }
                    {
